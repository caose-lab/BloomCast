...
NEW:
- Adds --publish flag
- If --update-cms and --publish are set, publishes the site to the webflow.io subdomain
  after the CMS item update succeeds.
...
def parse_args():
    ...
    # NEW: Publish flag (webflow.io only, per your setup)
    parser.add_argument(
        "--publish",
        action="store_true",
        help="Publish the Webflow site to the webflow.io subdomain after CMS update succeeds.",
    )
...
def publish_site_to_webflow_subdomain() -> dict:
    """
    Publish the site to its webflow.io subdomain.
    This is required for new Assets / CMS changes to reflect on the published site.
    """
    url = f"https://api.webflow.com/v2/sites/{SITE_ID}/publish"
    payload = {"publishToWebflowSubdomain": True}
    resp = requests.post(url, json=payload, headers=webflow_headers(), timeout=60)
...
def main():
    ...
    if args.update_cms:
        ...
        resp = requests.patch(cms_url, json=cms_payload, headers=webflow_headers(), timeout=60)
        if not resp.ok:
    ...
    # NEW: Publish (only makes sense after CMS update, per your workflow)
    if args.publish:
        if not cms_updated:
            print(
                "Note: --publish was set but CMS was not updated in this run. "
                "Skipping publish. (Use --update-cms --cms-item-id ... --publish)",
                file=sys.stderr,
            )
        else:
            try:
                out = publish_site_to_webflow_subdomain()
                print("Publish response:")
                print(json.dumps(out, indent=2))
            except requests.HTTPError as e:
                status = getattr(e.response, "status_code", "unknown")
                text = getattr(e.response, "text", "")
                print(f"Site publish failed: {status} {text}", file=sys.stderr)
                raise
