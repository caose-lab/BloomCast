...
METEO_COLS = [
    "wind_avg", "precipitation",
    "air_temperature", "temp_max", "temp_min",
    "air_pressure", "Watt_per_m2", "PAR",
    "water_temperature", "water_level"
]

OPTIC_COLS = [
    "CHL_NN","CHL_OC4ME", "TSM_NN", "ADG443_NN", "KD490_M07",
    "T865", "A865",
    "rho_665", "rho_681", "rho_709",
    "CIcyano",
...
def build_features(df: pd.DataFrame, expected: Optional[Sequence[str]] = None) -> pd.DataFrame:
    ...
    Notebook-compatible feature builder:
      - ensures date/datetime
      - ensures CHL_NN exists (accepts CHL_NN_total alias)
      - rolling stats on METEO_COLS + OPTIC_COLS using shift(1)
        stats: mean, std, p95, iqr (+ skew/kurt for w>=28)
      - lags of CHL_NN at {7,14,30,60}: CHL_NN_lag7, ...
      - optional target rolling stats ONLY if training included them (detected via expected)
      - optional calendar splits ONLY if training included them (detected via expected)
    """
    df = df.copy()
    ...
    # 2) Ensure CHL_NN exists (align with notebook naming)
    if "CHL_NN" not in df.columns:
        if "CHL_NN_total" in df.columns:
            df["CHL_NN"] = df["CHL_NN_total"]
        elif "CHLL_NN_TOTAL" in df.columns:
            df["CHL_NN"] = df["CHLL_NN_TOTAL"]

    # --- ensure wind_avg exists (training expects wind_avg; input uses AWND) ---
    if "wind_avg" not in df.columns and "AWND" in df.columns:
        df["wind_avg"] = pd.to_numeric(df["AWND"], errors="coerce")

    # 3) (Optional) Build rho_* and CIcyano if reflectances exist and these are missing
    refl_needed = ["Oa08_reflectance", "Oa10_reflectance", "Oa11_reflectance"]
    if all(c in df.columns for c in refl_needed):
        # numeric coercion
        for c in refl_needed:
            df[c] = pd.to_numeric(df[c], errors="coerce")

        if "rho_665" not in df.columns:
            df["rho_665"] = df["Oa08_reflectance"] * np.pi
        if "rho_681" not in df.columns:
            df["rho_681"] = df["Oa10_reflectance"] * np.pi
        if "rho_709" not in df.columns:
            df["rho_709"] = df["Oa11_reflectance"] * np.pi

        if "CIcyano" not in df.columns and all(c in df.columns for c in ["rho_665","rho_681","rho_709"]):
            df["CIcyano"] = -(
                df["rho_681"]
                - df["rho_665"]
                + ((df["rho_665"] - df["rho_709"]) / (709 - 665)) * (681 - 665)
            )
            df["CIcyano"] = df["CIcyano"].clip(lower=0)

    # 4) Rolling features (shifted by 1 inside) — NO median, NO E_v3
    ...
    # remove median stats to match notebook
    feats_meteo = feats_meteo.drop(columns=[c for c in feats_meteo.columns if "_median_" in c], errors="ignore")
    feats_optic = feats_optic.drop(columns=[c for c in feats_optic.columns if "_median_" in c], errors="ignore")

    # remove E_v3 if it was created
    for c in list(feats_meteo.columns):
        if "E_v3_" in c:
            feats_meteo = feats_meteo.drop(columns=[c], errors="ignore")

    # 5) Lags of CHL_NN exactly like notebook
    lags = pd.DataFrame(index=df.index)
    if "CHL_NN" in df.columns:
        for k in (7, 14, 30, 60):
            lags[f"CHL_NN_lag{k}"] = pd.to_numeric(df["CHL_NN"], errors="coerce").shift(k)

    # 6) Optional calendar splits (only if training expected them)
    cal = pd.DataFrame(index=df.index)
    if expected is not None:
        cal_needed = set(expected) & {"dow", "month", "day"}
        if cal_needed:
            cal["dow"]   = df["date"].dt.dayofweek
            cal["month"] = df["date"].dt.month
            cal["day"]   = df["date"].dt.day

    X = pd.concat([feats_meteo, feats_optic, lags, cal], axis=1)
    return X
...
def run_prediction(
    ...
    log.info(f"Feature audit: expected={len(expected)} built={X.shape[1]} missing={len(missing)} extra={len(extra)}")
    if missing:
        log.warning(f"Missing features (first 30): {missing[:30]}")
